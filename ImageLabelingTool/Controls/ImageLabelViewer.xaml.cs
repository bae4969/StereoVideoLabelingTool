	...
	public partial class ImageLabelingViewer : ImageControlBase
	{
		...
		public override unsafe void OnLoadImageInfo(ImageInfoType img_info, Action on_update_image_info) {
			...
			if (Application.Current.Dispatcher.CheckAccess() == false)
				Application.Current.Dispatcher.Invoke(() => { OnLoadImageInfo(img_info, on_update_image_info); });
			else {
				try {
		...
		public override unsafe void OnUpdateImageInfo() {
			...
			if (Application.Current.Dispatcher.CheckAccess() == false)
				...
				if (!__img_info.GetXmlValue("LUT", "MIN", out float t_min)) t_min = 0;
				if (!__img_info.GetXmlValue("LUT", "MAX", out float t_max)) t_max = 65535;
				if (!__img_info.GetXmlValue("LUT", "BLEND", out float t_blend)) t_blend = 0.3f;
		...
		private void ImageControlBase_SizeChanged(object sender, SizeChangedEventArgs? e) {
			if (__img_info == null ||
				__img_info.ImgSize.w <= 0 ||
				__img_info.ImgSize.h <= 0 ||
				__img_info.ImgSize.d <= 0)
				return;
		...
		private void OnChangeImageRotate(POINT3 cur_pos) {
